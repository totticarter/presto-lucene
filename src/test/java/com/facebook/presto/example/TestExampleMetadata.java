...
public class TestExampleMetadata
{
    ...
    private static final LuceneTableHandle NUMBERS_TABLE_HANDLE = new LuceneTableHandle(CONNECTOR_ID, "example", "numbers");
    private LuceneMetadata metadata;
            ...
            throws Exception
    {
        ...
        LuceneClient client = new LuceneClient(new LuceneConfig().setMetadata(metadataUri), CATALOG_CODEC);
        metadata = new LuceneMetadata(new LuceneConnectorId(CONNECTOR_ID), client);
    }
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(SESSION, NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new LuceneColumnHandle(CONNECTOR_ID, "text", createUnboundedVarcharType(), 0),
                "value", new LuceneColumnHandle(CONNECTOR_ID, "value", BIGINT, 1)));

        // unknown table
        try {
            metadata.getColumnHandles(SESSION, new LuceneTableHandle(CONNECTOR_ID, "unknown", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
        try {
            metadata.getColumnHandles(SESSION, new LuceneTableHandle(CONNECTOR_ID, "example", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(SESSION, new LuceneTableHandle(CONNECTOR_ID, "unknown", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new LuceneTableHandle(CONNECTOR_ID, "example", "unknown")));
        assertNull(metadata.getTableMetadata(SESSION, new LuceneTableHandle(CONNECTOR_ID, "unknown", "numbers")));
    }
    ...
    {
        assertEquals(metadata.getColumnMetadata(SESSION, NUMBERS_TABLE_HANDLE, new LuceneColumnHandle(CONNECTOR_ID, "text", createUnboundedVarcharType(), 0)),
                new ColumnMetadata("text", createUnboundedVarcharType()));
