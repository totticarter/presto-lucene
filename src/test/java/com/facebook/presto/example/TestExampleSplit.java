...
{
    private final LuceneSplit split = new LuceneSplit("connectorId", "schemaName", "tableName", URI.create("http://127.0.0.1/test.file"));

    @Test
    public void testAddresses()
    {
        // http split with default port
        LuceneSplit httpSplit = new LuceneSplit("connectorId", "schemaName", "tableName", URI.create("http://example.com/example"));
        ...
        httpSplit = new LuceneSplit("connectorId", "schemaName", "tableName", URI.create("http://example.com:8080/example"));
        assertEquals(httpSplit.getAddresses(), ImmutableList.of(HostAddress.fromParts("example.com", 8080)));
        assertEquals(httpSplit.isRemotelyAccessible(), true);

        // http split with default port
        LuceneSplit httpsSplit = new LuceneSplit("connectorId", "schemaName", "tableName", URI.create("https://example.com/example"));
        ...
        // http split with custom port
        httpsSplit = new LuceneSplit("connectorId", "schemaName", "tableName", URI.create("https://example.com:8443/example"));
    ...
    {
        JsonCodec<LuceneSplit> codec = jsonCodec(LuceneSplit.class);
        String json = codec.toJson(split);
        LuceneSplit copy = codec.fromJson(json);
