...
public class TestExampleRecordSet
{
    ...
    public void testGetColumnTypes()
            throws Exception
    {
        RecordSet recordSet = new LuceneRecordSet(new LuceneSplit("test", "schema", "table", dataUri), ImmutableList.of(
                new LuceneColumnHandle("test", "text", createUnboundedVarcharType(), 0),
                new LuceneColumnHandle("test", "value", BIGINT, 1)));
        assertEquals(recordSet.getColumnTypes(), ImmutableList.of(createUnboundedVarcharType(), BIGINT));

        recordSet = new LuceneRecordSet(new LuceneSplit("test", "schema", "table", dataUri), ImmutableList.of(
                new LuceneColumnHandle("test", "value", BIGINT, 1),
                new LuceneColumnHandle("test", "text", createUnboundedVarcharType(), 0)));
        ...
        recordSet = new LuceneRecordSet(new LuceneSplit("test", "schema", "table", dataUri), ImmutableList.<LuceneColumnHandle>of());
