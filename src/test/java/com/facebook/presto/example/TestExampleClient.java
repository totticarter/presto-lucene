...
public class TestExampleClient
{
            ...
            throws Exception
    {
        ...
        LuceneClient client = new LuceneClient(new LuceneConfig().setMetadata(metadata), CATALOG_CODEC);
        ...
        LuceneTable table = client.getTable("example", "numbers");
        assertNotNull(table, "table is null");
        assertEquals(table.getName(), "numbers");
        assertEquals(table.getColumns(), ImmutableList.of(new LuceneColumn("text", createUnboundedVarcharType()), new LuceneColumn("value", BIGINT)));
